...
{ 
	struct queue *newQueue_node =kmalloc(sizeof(struct queue),GFP_KERNEL);
	int ret;
	newQueue_node->dp_urb = urb;
...
{   //write it into a file
	...
	struct file *fp;
//	struct queue *front = (struct queue *)data;
	unsigned long int actual_len=0,i=0,len=0;
	loff_t pos = 0 ;
	...
	set_current_state(TASK_UNINTERRUPTIBLE);/* changes the state of the currently executing process from TASK_RUNNING to TASK_INTERRUPTIBLE*/
	while (!kthread_should_stop()) {/*The thread cannot exit until the kthread_should_stop() function returns 1*/
...
//printk("actual length =%d\n",front->dp_urb->iso_frame_desc[i].actual_length);
	wake_up_interruptible(&wq);                                                                                                            
	mythread = kthread_run(threadfn_pop_queue,(void*)1,"kernel_thread");
	if (mythread)
