...
{   //write it into a file
    ...
    { 	//The thread cannot exit until the kthread_should_stop() function returns 1
        ...
        for(k=0 ; k < No_of_packets ; k++)
        {
            ...
            current_pos = test->iso_frame_desc[k].offset;
            if((actual_len != 0) &&(actual_len != 12))
            {
                ...
                if(actual_len <= temp_len){
                    len = vfs_write(fp, (char *)(test->transfer_buffer)+current_pos, actual_len, &fp->f_pos);
                    if (len < 0) {
                    ...
                    while(actual_len != 0) {
                        len = vfs_write(fp,(char*)(test->transfer_buffer)+current_pos,temp_len, &fp->f_pos);
