...
struct file *fp;
static int fflag=1;
void resubmit(struct urb *urb)
{
    int ret;
    printk("in resubmit urb fun\n");
...
void Enqueue(struct urb *urb)
{
    ...
    newNode = kzalloc(sizeof(struct queue),GFP_ATOMIC);
    if(newNode == NULL)
    {
...
{   //write it into a file
    ...
    { 	//The thread cannot exit until the kthread_should_stop() function returns 1
        ...
        else
        {
            ...
            int temp_len=1024;
            actual_len = front->dp_urb->iso_frame_desc[0].actual_length;
            current_pos = front->dp_urb->iso_frame_desc[0].offset;
            if((actual_len != 0) &&(actual_len != 12))
            {
                ...
                if(actual_len <= temp_len){
                    len = vfs_write(fp, (unsigned char *)(front->dp_urb->transfer_buffer)+current_pos,actual_len, &fp->f_pos);
                    if (len != actual_len) {
                    ...
                    while(actual_len != 0) {
                        len = vfs_write(fp,(unsigned char*)(front->dp_urb->transfer_buffer)+current_pos,temp_len, &fp->f_pos);
                        if (len != temp_len) {
                            printk(KERN_ERR "vfs_write failed\n");
                        }
            ...
            if ((ret = usb_submit_urb(front->dp_urb, GFP_ATOMIC)) < 0) {
                printk(KERN_ERR "Failed to resubmit video URB %d\n",ret);
            }
            //  kfree(front);
        }
