...
int Enqueue(void *u)
{
	...
	{ 	//The thread cannot exit until the kthread_should_stop() function returns 1
		...
		else
		{
			...
			printk("tail addr =%p :%d\n",tail,tail->dp_urb->iso_frame_desc[n].actual_length);
			printk("eq->urb actual_len =%d\n",urb->iso_frame_desc[n].actual_length);
		}
		printk("%s: called from this LINE:%d\n", __func__,__LINE__);
		printk("eq->head addr =%p: data:%d\n",head,head->dp_urb->iso_frame_desc[n++].actual_length);
		flag =1;
		wake_up_interruptible(&wq);
	}/* if(kthread_should_stop()) is true, then unlock the lock , getting all the other threads trying to fetch it stuck.*/ 
...
{   //write it into a file
	struct queue *head = (struct queue*)data;
	struct file *fp;
	unsigned long int actual_len=0,k=0,len=0,npkt=0;
	loff_t pos = 0 ;
...
//		wait_event_interruptible(wq, flag == 1);
				...
				printk("%s: called from this LINE:%d\n", __func__,__LINE__);
				printk("dq->temp->dp_urb actual_len =%d\n",head->dp_urb->iso_frame_desc[k].actual_length);
				npkt = head->dp_urb->number_of_packets;       
				for(k=0 ; k < npkt ; k++)
				{
					int temp_len=1024;
					printk("\n%s: called from this LINE:%d\n", __func__,__LINE__);
					actual_len = temp->dp_urb->iso_frame_desc[k].actual_length;
					current_pos = temp->dp_urb->iso_frame_desc[k].offset;
					printk("dq->temp->dp_urb actual_len =%d\n",temp->dp_urb->iso_frame_desc[k].actual_length);

					if((actual_len <=temp_len)){
					printk("\n%s: called from this LINE:%d\n", __func__,__LINE__);
						len = vfs_write(fp, (char *)(temp->dp_urb->transfer_buffer)+current_pos, actual_len, &fp->f_pos);
						pos = pos+len;
						current_pos = current_pos + len;
					}
					else{
						while((actual_len != 0) && (actual_len >temp_len)){
							printk("\n%s: for->while called from this LINE:%d\n", __func__,__LINE__);

							len = vfs_write(fp,(char*)(temp->dp_urb->transfer_buffer)+current_pos, temp_len, &fp->f_pos);
							if (len < 0) {
								printk(KERN_ERR "vfs_write failed\n");
							}
							actual_len = actual_len - temp_len;
							if(actual_len < temp_len)
								temp_len = actual_len;
							current_pos = current_pos + len;
							pos = pos + len;
...
//	Enqueue(urb,urb->iso_frame_desc[x++].actual_length);
	mythread1 = kthread_run(Enqueue,(void*)urb,"kernel_thread");
	if (mythread1)
...
/*-----thread------*/ 
	...
	mythread2 = kthread_run(threadfn_pop_queue,(void*)head,"kernel_thread");
	if (mythread2)
		printk(KERN_INFO "Thread2 Created successfully\n");
	else
