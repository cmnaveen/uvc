...
int copy_videoData_file(void *data)
{
...
//    mm_segment_t old_fs;
  ...
  //      set_fs(KERNEL_DS);
        ...
        No_of_packets = test->number_of_packets;       
        for(k=0 ; k < No_of_packets ; k++)
        {
            unsigned int actual_len=0,len=0;
            actual_len = test->iso_frame_desc[k].actual_length;
            current_offset = test->iso_frame_desc[k].offset;
            if((actual_len != 0) &&(actual_len != 12))
            {
             ...
             //   len = vfs_write(fp, (char *)(test->transfer_buffer)+current_offset, actual_len, &fp->f_pos);
                len = kernel_write(fp, (char *)(test->transfer_buffer)+current_offset, actual_len, &fp->f_pos);
                if (len < actual_len) {
    ...
    //    set_fs(old_fs);
        ...
        if ((ret = usb_submit_urb(test, GFP_ATOMIC)) < 0) {
...
/*-----------my kernel thread----------*/
 /*  mythread = kthread_run(copy_videoData_file,NULL,"kernel_thread");
   if (mythread)
