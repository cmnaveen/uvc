...
{ 
	struct queue *newNode =kmalloc(sizeof(struct queue),GFP_KERNEL);
	int ret;
	newNode->dp_urb = urb;
	...
	{
		struct queue *tem = head;
		rear->next = newNode; 
		rear = newNode;
		while(tem->next !=NULL)
			tem = tem->next;
		tem->next = newNode;
		printk("temp addr =%p :%d\n",tem,tem->dp_urb->iso_frame_desc[k++].actual_length);
		printk("rear addr =%p\thead addr =%p\n",rear,head);
	}
		printk("front address (%p)\n",front);                
		printk("urb actual len :%d\n",urb->iso_frame_desc[k++].actual_length);
	flag =1;
...
{   //write it into a file
	...
	struct file *fp;
	int actual_len=0,i=0,len=0,npkt=0;
	loff_t pos = 0 ;
...
//printk("actual length =%d\n",front->dp_urb->iso_frame_desc[i].actual_length);
	mythread = kthread_run(threadfn_pop_queue,(void *)front,"kernel_thread");
	if (mythread)
