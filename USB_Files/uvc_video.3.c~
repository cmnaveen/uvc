...
typedef struct Queuedata
{
	...
	struct urb *data_urb; 	// queue data
...
int IsEmpty(struct Queue *newNode)
{
  int   ret;
        spin_lock(&(newNode->lock));
        ret = list_empty(&(newNode->list));
        spin_unlock(&(newNode->lock));
        return ret;
}


struct Queue* qCreate(void)
{

struct Queue *new;//= (struct Queue*) Kzalloc(sizeof(struct Queue *),GFP_ATOMIC);
	if(!new)
	{
		printk(KERN_ALERT "kzalloc fail\n");
		return new;
	}
	INIT_LIST_HEAD(&(new->list)); // list head initialized 
	qflag=0;
	spin_lock_init(&(new->lock));
	return new;
}

void Enqueue(struct Queue *newNode, struct urb *urb)
{
	int ret;	
	QueueData *qdata;
	qdata = (QueueData *) kzalloc(sizeof(QueueData),GFP_ATOMIC);
	...
	qdata->data_urb = urb;
	spin_lock(&(newNode->lock));
	list_add_tail(&(qdata->list),&(newNode->list));
	spin_unlock(&(newNode->lock));
...
int threadfn_Dequeue(void *newnode)
{
	...
	set_current_state(TASK_UNINTERRUPTIBLE);/* changes the state of the currently executing 
						 *process from TASK_RUNNING to TASK_INTERRUPTIBLE*/
	while (!kthread_should_stop()) {/*The thread cannot exit until the 
		...
		while(1){
			...
			spin_lock(&(newNode->lock));
			qdata = list_first_entry(&(newNode->list),QueueData,list);

			printk("\n%s: called from this LINE:%d\n", __func__,__LINE__);
			npkt = qdata->data_urb->number_of_packets;       
			for(i=0 ; i < npkt ; i++)
			{
				unsigned long int atemp=0,temp_len=1024;
				actual_len = qdata->data_urb->iso_frame_desc[i].actual_length;
				current_pos = qdata->data_urb->iso_frame_desc[i].offset;
				printk("\n%s:while->for called from this LINE:%d\n", __func__,__LINE__);

				while((actual_len <=temp_len) || (temp_len != 0)){
				printk("\n%s:for-->while called from this LINE:%d\n", __func__,__LINE__);
					len = vfs_write(fp, qdata->data_urb->transfer_buffer+current_pos, temp_len, &fp->f_pos);
					if (len < 0) {
	...
	printk("%s: END called from this LINE:%d\n", __func__,__LINE__);
...
//printk("actual length =%d\n",front->dp_urb->iso_frame_desc[i].actual_length);
	wake_up_interruptible(&wq);                                                                                                            
	mythread = kthread_run(threadfn_Dequeue,(void*)newNode,"kernel_thread");
	if (mythread)
