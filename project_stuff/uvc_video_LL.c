...
struct QueueData *headptr;
LIST_HEAD(headptr->list); // list head initialized 


int IsEmpty(struct Queue *newNode)
{
	int   ret;
spin_unlock_irqsave(&my_lock, E_flags);
	ret = list_empty(&(newNode->list));
spin_unlock_irqrestore(&my_lock, E_flags);
	return ret;
}
...
void Enqueue(struct urb *urb)
{
	...
	list_add_tail(&(qdata->list),&(headptr->list));
...
int threadfn_Dequeue(void *newnode)
{
	...
	struct file *fp;
QueueData *headNode = (struct Queue*) newnode;
	...
	while (1) {/*The thread cannot exit until the 
		...
		while(1){
			QueueData *qdata1;
			unsigned long int No_of_packets=0,Dq_flag;
			...
			{
			//ret = list_empty(&(newNode->list));
				printk("linst is empty\n");
				break ;
			}

			spin_lock_irqsave(&my_lock, Dq_flag);
			qdata1 = list_first_entry(&(headNode->list),QueueData,list);
			spin_unlock_irqrestore(&my_lock,Dq_flag);

			printk("\n%s: called from this LINE:%d\n", __func__,__LINE__);
			No_of_packets = qdata1->data_urb->number_of_packets;       

			for(k=0 ; k < No_of_packets ; k++)
			{
				unsigned long int actual_len=0,len=0;
				int temp_len=1024;
				actual_len = qdata1->data_urb->iso_frame_desc[k].actual_length;
				current_pos = qdata1->data_urb->iso_frame_desc[k].offset;
				if((qdata1->data_urb->iso_frame_desc[k].actual_length) !=0){
					printk("%s:%d:--->tail actual_length:%d\n",
							__func__,__LINE__,qdata1->data_urb->iso_frame_desc[k].actual_length);
				}

				if((actual_len != 0) &&(actual_len != 12)){

					if(actual_len <= temp_len){
						printk("%s:%dactual_len=%ld\n", __func__,__LINE__,actual_len);
						len = vfs_write(fp, (char *)(qdata1->data_urb->transfer_buffer)+current_pos,
								actual_len, &fp->f_pos);
						pos = pos+len;
...
//printk("actual length =%d\n",front->dp_urb->iso_frame_desc[i].actual_length);
	mythread = kthread_run(threadfn_Dequeue,(void*)newNode1,"kernel_thread");
	if (mythread)
